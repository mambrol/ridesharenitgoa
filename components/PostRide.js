...
import { postRide } from '../lib/firebase'
...
export default function PostRide({ user, onSuccess }) {
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-5">
      ...
      {form.payment === 'paid' && (
        <Field label="Amount">
          <input className="inp" placeholder="e.g. ₹50 per person" value={form.payNote} onChange={e => set('payNote', e.target.value)} />
        </Field>
      )}

      {/* Note */}
      <Field label="Note (optional)">
        <input className="inp" placeholder="Extra info for co-riders" value={form.note} onChange={e => set('note', e.target.value)} />
      </Field>
