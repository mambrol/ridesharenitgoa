...
export default function Topbar({ user, tab, setTab, unreadMessages, pendingRequests, alertCount }) {
  ...
  const mobileTabs = [
    { id: 'browse',   label: 'Browse',   icon: SearchIcon },
    { id: 'post',     label: 'Post',     icon: PlusIcon },
    { id: 'messages', label: 'Messages', icon: ChatIcon,  badge: unreadMessages },
    { id: 'my',       label: 'My rides', icon: UserIcon,  badge: pendingRequests },
  ]
  ...
  return (
    <>
      {/* Desktop top bar */}
      <div className="hidden sm:block sticky top-0 z-20 bg-white border-b border-gray-100">
      ...
      {/* Mobile top header */}
      <div className="sm:hidden sticky top-0 z-20 bg-white border-b border-gray-100">
        <div className="flex items-center justify-between px-4 h-12">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 bg-brand rounded-md flex items-center justify-center">
              <CarIcon className="w-3.5 h-3.5 text-white" />
            </div>
            <span className="font-medium text-sm text-gray-900">NIT Goa RideShare</span>
          </div>
          <div className="flex items-center gap-2">
      ...
      <div className="sm:hidden fixed bottom-0 left-0 right-0 z-20 bg-white border-t border-gray-100 safe-area-pb">
        <div className="flex">
          {mobileTabs.map((t) => (
            <button
              key={t.id}
              ...
              className={`flex-1 flex flex-col items-center justify-center py-2 relative transition-colors
                ${tab === t.id ? 'text-brand' : 'text-gray-400'}`}
            >
